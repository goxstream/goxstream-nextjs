...
1. **Generating Migrations**:
   - Make your changes to schema files in `src/infrastructure/database/drizzle/schema/`.
   - Run drizzle-kit to generate a migration `.sql` file:
     ```bash
     npx drizzle-kit generate
     ```
   - This writes a file into the migrations directory (e.g. `drizzle/migrations/0000_xxxx.sql`).
