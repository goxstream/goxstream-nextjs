...
async function getAnimeDetails(slug: string): Promise<{
  anime: Anime;
  episodes: any[];
}> {
  ...
  try {
    ...
    if (list.length > 0) {
      ...
      episodeRecords = await db
        .select()
        .from(episodesTable)
        .where(eq(episodesTable.animeId, animeRecord.id))
...
export default async function AnimeDetailsPage({ params }: PageProps) {
  ...
  const { anime, episodes } = await getAnimeDetails(slug);
